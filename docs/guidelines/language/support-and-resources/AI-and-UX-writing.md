...
> UX writers can now use ready-made prompts to create chatbots that rapidly generate and refine microcopy while ensuring content is checked for accuracy and alignment with guidelines.
...
* Output:
    ...
    * Action: Pause current operation. Recalibrate using calibration kit #TX45. Confirm green status light before resuming.
...
* Output:
    ...
    * Action: Check network cables on machine interface panel. Verify server status at portal.example.com/status. Retry operation after 2 minutes.
