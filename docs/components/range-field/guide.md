...
Range fields work as a wrapper component when two related inputs describe one bounded interval with a clear start and end. They are often used with date, time or date-time inputs, but they also work with other paired inputs such as number inputs for defining minimum and maximum allowed values.
...
- **Labels**: Label both inputs clearly, for example 'Start date' and 'End date' or 'From' and 'To'. Keep labels short when the surrounding heading already explains the context.
- **Paired input type**: Use the same input type in both fields so the range stays predictable. Combine for example two date inputs, two time inputs or two date-time inputs.
