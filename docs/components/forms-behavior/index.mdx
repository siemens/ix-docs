...
Single-line input controls (e.g. [text input](./input) and [number input](./number-input)) submit their parent form when the user presses Enter while focused. This behaviour speeds up common workflows such as quick searches or single-field actions.
...
- Con: Not suitable for multiline inputs (textarea) or multi-field forms that require review — can cause accidental submissions.
- Con: May submit partial or unintended data and make it harder for users to review before sending.
- Con: Can introduce accessibility issues if keyboard-only and screen-reader workflows aren't verified; ensure behavior is predictable and communicated to assistive technology users.
