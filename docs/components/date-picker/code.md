...
| from | The selected starting date. If the date-picker-rework is not in range mode this is the selected date. Format has to match the \`format\` property. | from | string \| undefined |  |
...
| to | The selected end date. If the the date-picker-rework is not in range mode this property has no impact. Format has to match the \`format\` property. | to | string \| undefined |  |
