...
- **Format**: Specify the time format to ensure that times are entered in a consistent and recognizable format. Default is ‘TT’ which is the localized 24-hour time with seconds  (read more in the [UX Writing guides](../../guidelines/language)).
...
<DoAndDont>
  <DoAndDont.Do>
    ...
    <DoAndDont.Item>Do use separate inputs for start and end times to simplify time ranges</DoAndDont.Item>
  ...
  <DoAndDont.Dont>
    <DoAndDont.Item>Don't use ambiguous formats without giving clear context</DoAndDont.Item>
  </DoAndDont.Dont>
</DoAndDont>
